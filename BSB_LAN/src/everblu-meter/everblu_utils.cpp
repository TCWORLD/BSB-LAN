...
void show_in_hex(uint8_t *buffer, size_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        if (!(i % 16)) writelnToDebug();
        printFmtToDebug("%02X ", buffer[i]);
    }
    writelnToDebug();
}
...
void show_in_hex_array(uint8_t *buffer, size_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        if (!(i % 16) && i > 0) writelnToDebug();
        printFmtToDebug("0x%02X, ", buffer[i]);
    }
    writelnToDebug();
}
...
int encode2serial_1_3(uint8_t *inputBuffer, int inputBufferLen, uint8_t *outputBuffer)
{
    ...
    for (i = 0; i < (inputBufferLen * 8); i++)
    {
        ...
        if (i % 8 == 0)
        {
            ...
            bytepos = j / 8;
            bitpos = j % 8;
            // printFmtToDebug(" j=%u startBIT",j);
            outputBuffer[bytepos] &= ~(1 << (7 - bitpos));
            j++;
        } // start stop bit
